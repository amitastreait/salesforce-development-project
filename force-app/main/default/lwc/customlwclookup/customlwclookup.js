...
export default class Customlwclookup extends LightningElement {
        ...
        .then(data => {
            if (data) {
                ...
                for (let i = 0; i < searchRecordList.length; i++) {
                    ...
                    if(record[this.customerIdField]){
                        record.secondField = record[this.customerIdField];
                    }   
                }
    ...
    handleSelect(event) {
        ...
        //window.console.log(' selectedVal ', JSON.stringify(selectedVal));
        let finalRecEvent = new CustomEvent('select', {
            detail: { selectedRecordId: this.selectedRecord.Customer_Id__c, parentfield: this.parentidfield }
        });
  ...
  @api 
  validateLookup(){
    let searchComp = this.template.querySelector('c-lwcsearchcomponent');
    let allValid = true;
    if(searchComp){
        allValid = searchComp.validateLookup();
    }
    return allValid;
  }
